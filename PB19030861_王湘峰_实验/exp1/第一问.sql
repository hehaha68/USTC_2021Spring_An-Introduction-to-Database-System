CREATE TABLE `exp1`.`Book` (
  ...
  CREATE TABLE `exp1`.`Borrow` (
  `Book_ID` CHAR(8) NOT NULL,
  `Reader_ID` CHAR(8) NOT NULL,
  `Borrow_Date` DATE NULL DEFAULT NULL,
  `Return_Date` DATE NULL DEFAULT NULL,
  PRIMARY KEY (`Book_ID`, `Reader_ID`),
  INDEX `r_idx` (`Reader_ID` ASC) VISIBLE,
  CONSTRAINT `b`
    FOREIGN KEY (`Book_ID`)
    REFERENCES `exp1`.`book` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `r`
    FOREIGN KEY (`Reader_ID`)
    REFERENCES `exp1`.`reader` (`ID`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION);
