...
alter table Employee add constraint FK_Manager foreign key (Emp_E_ID)
...
alter table Own add constraint FK_Own2 foreign key (C_ID)
      references Client (C_ID) on delete restrict on update restrict;

alter table Own add constraint FK_Own3 foreign key (B_Name)
      references Bank (B_Name) on delete restrict on update restrict;

alter table Own add constraint FK_Own4 foreign key (A_ID)
      references Account (A_ID) on delete restrict on update restrict;

alter table Payment add constraint FK_Pay foreign key (L_ID)
      references Loan (L_ID) on delete restrict on update restrict;
